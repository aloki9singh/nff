...
//user online/offline
export const getUserOnlineStatus = async (uid) => {
  const userStatusRef = dref(rdb, `/status/${uid}`);
  ...
  try {
    ...
    if (snapshot.exists()) {
      ...
      if (data.state === 'online') {
        ...
        const lastSeen = new Date(parseInt(data.last_changed, 10));
        const currentDate = new Date();

        if (
          lastSeen.toDateString() === currentDate.toDateString() &&
          lastSeen.getDate() === currentDate.getDate() &&
          lastSeen.getMonth() === currentDate.getMonth() &&
          lastSeen.getFullYear() === currentDate.getFullYear()
        ) {
          ...
          const formattedDate = `${lastSeen.getDate()}/${lastSeen.getMonth() + 1}/${lastSeen.getFullYear()}`;
...
export const getActivityLogsForUser = async (userID) => {
  const activityLogsRef = dref(rdb, `/activityLogs/${userID}`);
  ...
  try {
    ...
    snapshot.forEach((activitySnapshot) => {
      ...
      const endTime = activity.endTime ? new Date(activity.endTime) : new Date(); // Use current time if no endTime

      // Extract date parts (without time)
      const startDate = new Date(startTime.getFullYear(), startTime.getMonth(), startTime.getDate()).toLocaleDateString();

      // Calculate duration in seconds
      const durationInSeconds = Math.floor((endTime - startTime) / 1000);

      if (activityLogsData[startDate]) {
        activityLogsData[startDate] += durationInSeconds;
      } else {
        activityLogsData[startDate] = durationInSeconds;
      }
    });

    console.log('User activity logs data:', activityLogsData);
