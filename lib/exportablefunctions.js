...
import { callVerificationEmailApi, callVerificationEmailApiMentor } from "./api";
...
export const signUpMentor = async (mentorLogData, router, setLoading, role, setRole) => {
          ...
          callUserById(user.uid).then(async (res) => {
            ...
            if (["admin", "student", "school"].includes(value.user.role)) {
              ...
              if (res.success === false) {
                ...
                alert("Please verify your email! An email verification has been sent to your inbox.");
...
export const signUpStudent = async (email, password, router, setLoading, role, setRole) => {
  setLoading(true);
...
// ////  change provided month in number
// export function getMonthNumber(monthName) {
//   const monthMap = {
//     January: 1,
//     February: 2,
...
// //send schedule to mentorSchedule
...
// //get link by id from schedule collection
// export const getLinkById = async (docId) => {
//   try {
//     const docRef = doc(db, "mentorsSchedule", docId);
//     const docSnap = await getDoc(docRef);
