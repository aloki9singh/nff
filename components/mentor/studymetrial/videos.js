...
function Videos() {
	...
	return (
		<div>
			...
			<div className="grid lg:grid-cols-2 justify-center items-center  md:items-start md:ml-10   flex-col  gap-y-5 py-7">
				...
				{selectedModule.video.map((video, i) => (
					<>
          {/* For Teseting with more Videos duplicate whole of the div below this comment */}
						<div
							key={i}
							className=" flex  items-center flex-row gap-5 sm:gap-8 lg:gap-5 w-full justify-center sm:justify-start "
						>
							<div className="flex justify-between items-center">
								<div className="m-2 p-3 border rounded-2xl bg-gradient-to-r from-[#673CAF] to-[#DA2E8B] h-32 w-48 flex flex-col  group relative ">
									...
									<div>
										<h1 className="text-white text-sm sm:text-base font-medium mb-2 ">
											{selectedModule.name}
										</h1>
							...
							<div className=" flex flex-col gap-6 mt-4 items-center md:items-start text-sm sm:text-base">
								<div>{video.name}</div>
								<div className="w-full text-left">{video.subtitle}</div>
							</div>
						</div>
					</>
				))}
