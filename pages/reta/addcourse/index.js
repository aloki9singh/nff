...
const PlanCourseForm = ({ state, onSubmit }) => {
	...
	return (
		...
		<form onSubmit={handleSubmit(onSubmit)}>
			...
			{/* course name */}
			<div className="w-full flex flex-col md:flex-row justify-start space-y-5 md:space-y-0 md:items-center  md:gap-x-2 lg:px-4 my-8 ">
			...
			<div className="w-full  flex flex-col md:flex-row justify-start md:items-center space-y-5 md:space-y-0 lg:items-start  md:gap-x-2 lg:px-4 mb-8">
				<label className="md:w-32 lg:w-40 lg:pt-2" htmlFor="">
					Course Description
				</label>
				<div className="flex flex-col flex-1">
					<textarea
						...
						className={`  h-28 w-full rounded-lg px-2 ${
			...
			<div className="flex flex-col lg:flex-row justify-start items-start w-full space-x-0 lg:space-x-10 space-y-6 lg:space-y-0 lg:px-4 mb-8">
				<div className="flex flex-col md:flex-row flex-1 w-full md:items-center space-y-5 md:space-y-0 gap-x-2">
					<label className="md:w-32 lg:w-40" htmlFor="">
						Duration :
					</label>
					<div className="flex flex-col flex-1">
				...
				<div className="flex flex-col md:flex-row flex-1 md:items-center space-y-5 md:space-y-0 w-full gap-x-2 ">
					<label className="md:w-32 lg:w-40" htmlFor="">
						Lectures :
					</label>
					<div className="flex flex-col flex-1">
			...
			{/* level */}
			<div className="w-full relative  md:flex flex-col md:flex-row justify-start items-start lg:items-center gap-x-2 lg:px-4 pb-8">
				<p className="text-red-500 text-sm absolute left-48 bottom-2">
					{errors.level?.message}
				</p>

				<legend className="md:w-32 lg:w-40 shrink-0" htmlFor="">
					Level :
				</legend>
			...
			<div className="flex flex-col lg:flex-row justify-start items-start  space-x-0 lg:space-x-10 space-y-6 lg:space-y-0 lg:px-4 mb-8">
				<div className="flex flex-col md:flex-row flex-1 w-full md:items-center space-y-5 md:space-y-0 gap-x-2">
					<label className=" md:w-32 lg:w-40" htmlFor="">
						Category:
					</label>
					<div className="bg-[#313131] rounded-lg h-10 px-2 flex-1 w-full md:w-[217px] lg:w-full ">
						<select
				...
				<div className="flex flex-col md:flex-row flex-1 md:items-center space-y-5 md:space-y-0 w-full gap-x-2">
					...
					<div className="bg-[#313131] rounded-lg h-10 px-2 flex-1 w-full ">
						<select
							{...register("language")}
			...
			<div className="w-full flex flex-col md:flex-row justify-start items-start md:items-center gap-y-2 md:gap-x-2 lg:px-4 ">
				<label className="md:w-32 lg:w-40" htmlFor="">
					Upload Banner Image
				</label>
			...
			<div className="flex flex-col gap-2 text-sm text-white/80">
				...
				<ul className="flex flex-col gap-2 list-disc	">
					{Array.isArray(module.video) ? (
						...
						<p className="text-white/80 truncate">{module.video}</p>
					)}
				</ul>
			</div>
		</Accordian>
...
const ModuleForm = ({ editModule, onSubmit, setEditModule }) => {
	const {
		register,
		control,
		handleSubmit,
	...
	useEffect(() => {
		if (editModule) {
			reset({
				...editModule,
				video: editModule.video.map((v) => ({ value: v })),
			});
			return;
		}
...
const CourseContentForm = ({ initialModules = [], onSubmit }) => {
	...
	//   );
	// };
	console.log("editeditModule", editModule);
	const onModuleSubmit = (data) => {
		console.log("data", data);

		console.log("editModule", editModule);

		if (editModule) {
			setModules((prev) =>
				prev.map((m) => {
					if (m.name === editModule.name) return data;
					return m;
				})
			);
			setEditModule(null);
							...
							<div className="w-full flex flex-col mt-4 gap-y-3">
								{modules.map((module, index) => (
									<div key={index} className="flex flex-row items-center w-full gap-2">
										...
										<button
											className=" transition-colors duration-150 hover:text-green-500 p-2  "
											onClick={() => {
										...
										<button
											className="  transition-colors duration-150 hover:text-red-500 p-2 "
											onClick={() => {
												// show confirmation modal

												setModules(modules.filter((m, i) => i !== index));
											}}
										>
											<MdDelete className="text-lg" />
										</button>
        ...
        <div className=" flex justify-center gap-x-96 items-center">
          ...
          <div className="justify-start flex gap-2  sm:text-xs  md:text-md">
			...
			<div className="grid grid-cols-1 md:grid-cols-10 w-full">
				<div className="col-span-2 hidden md:block h-full px-4 md:pr-2">
					<Sidebar currentStep={currentStep} setStep={setStep} />
				</div>
				<div className="flex w-full col-span-8 min-h-screen md:pl-2 px-4 py-4">
					<div className="w-full h-full  p-5 md:p-8 bg-[#222222] rounded-lg ">
						{currentStep === 1 && (
							<PlanCourseForm
								state={formData}
